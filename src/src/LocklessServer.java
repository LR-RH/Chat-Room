...
/*
   FOR SETTINGS:
            run two client applications, have one change the name_colour setting and at the same time
            have one change the text_colour setting, exit settings on both clients

   FOR WHISPER:
            set up three clients, have one client whisper the first block of text in "whispertest.txt"
            to the third client, whilst simultaneously having the second client whisper the second block
            of text to the third client.

    FOR CHATROOMS:

    FOR DISCONNECT:
            simultaneously disconnect two clients at the same time



 */

public class LocklessServer implements Runnable {
    ...
    public LocklessServer(Socket tcpClientSocket, String client) throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            try {
                ...
                new Thread(new LocklessServer(tcpClientSocket, client)).start();
    ...
    public void sendMessage(String message, String client, String roomName) throws IOException {
    ...
    public void sendWhisper(String message, String client, String recipient, String roomName) throws IOException {
        boolean found = false;
    ...
    public void processCommand(String inputLine, String client) throws IOException {
        BufferedReader in = new BufferedReader(new InputStreamReader(tcpClientSocket.getInputStream()));
        PrintWriter out = new PrintWriter(tcpClientSocket.getOutputStream(), true);
           ...
           // }
            ...
            //synchronized (chatRooms) {
                //synchronized (messageLock) {
                    ...
                    for (int i = 0; i < clients.size(); i++) {
                        if (chatRooms.get(currentRoom).contains(clients.get(i))) {
                            membersArray[i] = "- " + clients.get(i);
                        }
                    }
                //}
    ...
    public void settings(String inputLine, String client, BufferedReader in) throws IOException {
        PrintWriter out = new PrintWriter(tcpClientSocket.getOutputStream(), true);;
       ...
       // if (settingslock.tryLock()) {
            ...
            try {
                ...
                while (cont) {
                    ...
                    } else if (chosenSetting.equalsIgnoreCase("exit")) {
                        sendQueue();
    ...
    public void createRoom(String roomName, PrintWriter out) throws IOException {
        //synchronized (chatRooms) {
    ...
    public void joinRoom(String roomName, PrintWriter out) throws IOException {
        //synchronized (chatRooms) {
            chatRooms.get(currentRoom).remove(tcpClientSocket);
    ...
    public void leaveRoom(PrintWriter out) throws IOException {
        //synchronized (chatRooms) {
